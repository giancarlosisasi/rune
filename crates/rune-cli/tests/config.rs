...
fn cache_clear_removes_the_directory_and_is_idempotent() {
  let test = Test::new().config(CONFIG);
  let root = test.dir().to_path_buf();

  // Populate the cache by loading once.
  Test::new().config(CONFIG).args(["list"]).stdout_regex(r"(?s)build").status(0).run();
  ...
  Test::new().config(CONFIG).args(["cache", "clear"]).stdout("").stderr("").status(0).run();
}
