...
description: 'What a rune.config.ts may import and call, the frozen rune global, and how the resolved result is cached by import-graph content.'
---
...
`@giancarlosio/rune` is answered from inside the binary rather than resolved from `node_modules`, so
