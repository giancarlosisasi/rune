...
When no `package.json` can be found, nothing is written and the message says which of the two ends
the search reached, because they need different answers.
...
add a package.json to this repository, or write a config without seeding:

  rune init
```

Reaching the filesystem root instead means you are not in a project at all, and the message says to
change into one first. `rune init` on its own works in both cases.
